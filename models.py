...
def insertUser(request):
    ...
    if not row:
        cur.execute("INSERT INTO users (username,password) VALUES (?,?)", (request.form['username'], sha256_crypt.encrypt(request.form['password'])))
        con.commit()
...
def playlist(uname , sid, sname, p_name):
    try:
        con = sql.connect("database.db")
        print "db connectivity successfull"
        cursor = con.cursor()
...
def fetch_hiphop():
    try:
        con = sql.connect("database.db")
        print "db connectivity successfull"
        sqlQuery1 = "select * from song where sgenreid in (2,3,4,6,7,9,10,11,12,14,15)"
